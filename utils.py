...
from dataset_physio import get_testloader_physio
...
def evaluate_imputation_all(models, mse_folder, dataset_name='', batch_size=16, trials=10, length=-1, random_trial=False, forecasting=False, missing_ratio=0.1, test_indices=None, data=False):  
    nsample = 50
    if 'CSDI' in models.keys():
        models['CSDI'].eval()
    if 'DiffSAITS' in models.keys():
    ...
    for trial in range(trials):
        ...
        if dataset_name == 'synth':
            ...
            test_loader = get_testloader_synth(n_steps=100, n_features=7, batch_size=batch_size, num_seasons=16, seed=(10 + trial), length=length, missing_ratio=missing_ratio, random_trial=random_trial, forecasting=forecasting, v2=True)
